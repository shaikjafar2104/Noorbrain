(() => {
  ...
  const state = {cameras:[],activeCamera:null,devices:[],installPrompt:null,recorder:null,stream:null,chunks:[],recording:false,cameraMode:"clean",cameraFallback:0,cameraRecorder:null,cameraChunks:[],cameraRecording:false,cameraCanvas:null,cameraDrawTimer:0};
  ...
  function showCamera(camera) {
    ...
    const streamUrl=state.cameraMode==="vision"?"/vision_feed":(state.cameraFallback===0?"/camera_live":"/camera_feed");
    ...
    image.onerror=()=>{
      if(state.cameraMode==="clean"&&state.cameraFallback===0){
        state.cameraFallback=1;setCameraNotice("Direct feed unavailable; trying local feed");showCamera(camera);return;
      }
      if(state.cameraMode==="clean"&&state.cameraFallback===1){
        state.cameraFallback=0;state.cameraMode="vision";updateOverlayButton();showCamera(camera);return;
      }
  ...
  function toggleCameraOverlay(){state.cameraMode=state.cameraMode==="vision"?"clean":"vision";state.cameraFallback=0;updateOverlayButton();showCamera(state.activeCamera);}
  ...
  async function loadConfig(){
    ...
    state.cameras=[{id:"noorbrain-camera",name:"Home Camera",room:"Home",stream_url:"/camera_live"}];
  ...
  async function sendHalo(command){
    const reply=document.querySelector("#nbv2HaloReply");
    ...
    try{
      ...
      const p=await request(
        "/api/universal-voice-v9/command",
        {
          method:"POST",
          body:JSON.stringify({
            text:message,
      ...
      if(p.status==="duplicate"){
        reply.textContent="Already received.";
        return;
      }
  ...
  const modules={
    ...
    media:"/studio#media-library",voice:"/studio#halo-speak",settings:"__NOOR_SETTINGS__"
  };
  ...
  document.addEventListener("click", function(event){
    ...
    if(button.dataset.module !== "settings") return;
    ...
    if(window.NoorMobileSettingsV11?.open){
      ...
      console.error("NOOR SETTINGS MODULE NOT READY");
    }
  }, true);
