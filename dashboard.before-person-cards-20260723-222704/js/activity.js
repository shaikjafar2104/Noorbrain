(() => {
  ...
  function buildPage() {
    ...
    if (
      nav &&
      !nav.querySelector('[data-page="activity"]')
    ) {
      ...
      const timelineButton =
        nav.querySelector('[data-page="timeline"]');

      if (timelineButton) {
        nav.insertBefore(button, timelineButton);
      } else {
    ...
    if (!document.getElementById("page-activity")) {
        ...
        <article class="card">
          <div class="card-head">
            <div>
              ...
              <p>
                Live person-aware events from NoorBrain
              </p>
            </div>
      ...
      document
        .getElementById("activityRefresh")
        ?.addEventListener("click", loadActivities);

      document
        .getElementById("activityClear")
  ...
  function openActivityPage() {
    ...
    if (subtitle) {
      subtitle.textContent =
        "Live person movement and presence events";
    }

    loadActivities();

    clearInterval(refreshTimer);
    refreshTimer = setInterval(loadActivities, 2000);
  }
  ...
  async function loadActivities() {
    ...
    try {
      ...
      document.getElementById(
        "activityActiveCount"
      ).textContent = data.active_count ?? 0;

      document.getElementById(
        "activityEventCount"
      ).textContent = data.event_count ?? 0;

      document.getElementById(
        "activityStatus"
      ).textContent = data.status || "running";
      ...
      if (!events.length) {
        ...
        list.innerHTML =
          "No activity events yet. Move in front of the camera.";
        return;
      }
      ...
      list.innerHTML = events.map(event => `
        <div class="activity-row">
          <div class="activity-time">
            ${escapeHtml(timeOnly(event.timestamp))}
          </div>
      ...
      if (list) {
        list.className = "activity-empty";
        list.textContent =
          `Activity API unavailable: ${error.message}`;
      }
    }
  }
