...
def get_raw_encoded_frame():
    """Return the camera frame without zones, boxes, labels or debug text."""
    global _raw_jpeg_bytes, _raw_jpeg_time

    now = time.monotonic()
    with _raw_jpeg_lock:
        if (
            _raw_jpeg_bytes is not None
            and now - _raw_jpeg_time < _jpeg_interval
        ):
            return _raw_jpeg_bytes
        ...
        _raw_jpeg_bytes = buffer.tobytes()
        _raw_jpeg_time = now
        return _raw_jpeg_bytes
...
@app.get("/camera_feed")
def camera_feed():
    """Clean live camera stream for the mobile and web product UI."""
    return StreamingResponse(
        generate_raw(),
        media_type="multipart/x-mixed-replace; boundary=frame",
        headers={
...
# ISLAMIC DUA AND AZKAR AUDIO RULES
from services.islamic_audio_rules.routes import router as islamic_audio_rules_router
app.include_router(islamic_audio_rules_router)


# NOORBRAIN SMART PRESENCE V16
from services.person_ai.gallery_routes import router as person_gallery_router
app.include_router(person_gallery_router)


# NOORBRAIN PI HALO WAKE V16
from services.pi_wake_v16.routes import router as pi_wake_v16_router
app.include_router(pi_wake_v16_router)


# NOORBRAIN ISLAMIC AUDIO CONTROL BRIDGE v16.1
from services.islamic_audio_control.routes import router as islamic_audio_control_router
app.include_router(islamic_audio_control_router)
